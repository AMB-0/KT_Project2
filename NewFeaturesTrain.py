...
punctuation = "[\\\"#$%&'()*+,-./:;<=>@[\]^_`{|}~]"
num_attr = 94
path = 'C:\\Users\\XXXX\\Knowledge Technologies (COMP90049)\\Project 2\\Assignment\\'
...
#-------------------------------------------------------------------------------------------------------------
#								 STEP 1: READING THE RAW TRAINING SET
#-------------------------------------------------------------------------------------------------------------

# We read the training set raw data
raw_train_set = pd.read_csv(path + 'train\\train.txt', sep = '	', header = None, skiprows=0 , quoting=csv.QUOTE_NONE, error_bad_lines=False)
...
for tweets in raw_train_set.values[:,2]:
...
# We read the given train dataset and extract the header from it
header = pd.read_csv(path + 'train\\train.arff', sep = ' ', header = None, skiprows=1 )


# Then, we read the training set in the test.arff file, we add the header to it and save it in a pandas dataframe
train_set = pd.read_csv(path + 'train\\train.arff', sep = ',', header = None, skiprows=96, quoting=csv.QUOTE_NONE, error_bad_lines=False, names = list(header.values[:num_attr,1]))
del train_set["id"]

# We add the features to the new pandas dataframe
train_set["sentiment_polarity"] = np.array(sentiment_polarity)												# Adds polarity to pandas dataframe
train_set["tweet_length"] = np.array(length)																# Adds tweet length to pandas dataframe
train_set["punctuation_ind"] = np.array(punctuation_ind)													# Adds punctuation indicator to pandas dataframe
train_set["drugs_side_effect_ind"] = np.array(drugs_side_effect_ind)										# Adds drugs and side effect indicator to pandas dataframe


# Export the final dataframe to a CSV file
train_set.to_csv(path + 'train\\out.csv', sep=",", index=False)
